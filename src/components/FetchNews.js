...
export default function FetchNews() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      setIsLoading(true);
      const url = `https://hn.algolia.com/api/v1/search_by_date?tags=story&query=${query}&numericFilters=points>0`;
      const res = await fetch(url);
      const data = await res.json();
      const sortedItems = data.hits.sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
      );
      setItems(sortedItems);
      setLargeTitle(sortedItems[0]);
      setIsLoading(false);
    };
        ...
        {isLoading ? (
          ...
          <>
            <section className="homeScreen">
              {/*Heading*/}
              <h1 className="headingTitle" style={{ fontSize: '60px', fontFamily: 'Bungee, sans-serif', textAlign: 'center' }}>
  R<span style={{ color:'#9FEF00' }}>oo</span>kle
</h1>
